...
#'     variable = reactive(dataset$data_var_x))
#' }
show_RF_model <- function(input, output, session, PCA, RF_btn, updated) {
  ns <- session$ns
  ...
  observeEvent(RF_btn(), {
    ...
    algo_btn1_result$importance <- impo
    algo_btn1_result$fimp0 <- fimp0
    algo_btn1_result$All_data <- All_data
    algo_btn1_result$classifier <- mo
  })


  output$classifier <- renderPrint({
    print(algo_btn1_result$classifier)
  })



  output$variable_plot <- renderPlot({
    req(algo_btn1_result$fimp0)
    fimp0 <- algo_btn1_result$fimp0
  ...
  observeEvent(input$variableToShow_btn, {
    ...
    All_data_ggAni <- na.omit(updated()) %>% dplyr::select(-c(System.Recipe))
    y <- "None"
    All_data_ggAni$y <- y
    # Convert data to a "tall" format
  ...
  # 
  #   data$Start.Time <- as.Date(data$Start.Time, format = "%Y/%m/%d %H:%M", tz = "GMT")
  # 
